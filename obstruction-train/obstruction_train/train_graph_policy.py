...
def train_epoch(records, model, encoder, optimizer, device):
    model.train()
    encoder.train()
    total_loss = 0.0
    correct = 0
    for record in records:
        object_ids, nodes, edges = build_graph(record["objects"], record.get("relations", []), record.get("target_id"))
        ...
        logits = model(nodes[None].to(device), edges[None].to(device), torch.ones(1, len(object_ids), dtype=torch.bool, device=device), torch.tensor([task], device=device), target_mask, encoder([record.get("instruction", "")]))
...
def main():
    ...
    encoder = HashTaskEncoder(embedding_dim=args.text_dim).to(args.device)
    optimizer = torch.optim.AdamW(list(model.parameters()) + list(encoder.parameters()), lr=args.lr)
    output = Path(args.out_dir)
    output.mkdir(parents=True, exist_ok=True)
    best_accuracy = -1.0
    for epoch in range(1, args.epochs + 1):
        loss, accuracy = train_epoch(records, model, encoder, optimizer, args.device)
        print(f"epoch={epoch} loss={loss:.4f} action_accuracy={accuracy:.4f}", flush=True)
        if accuracy >= best_accuracy:
            best_accuracy = accuracy
            torch.save({"model": model.state_dict(), "text_encoder": encoder.state_dict(), "config": config, "text_encoder_config": {"embedding_dim": args.text_dim, "num_buckets": encoder.num_buckets}, "epoch": epoch, "action_accuracy": accuracy}, output / "best.pt")
