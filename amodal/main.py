...
def parse_args():
    ...
    parser.add_argument('--modal_masks_dir',   type=str,  help='Optional folder with precomputed modal masks and metadata')
    ...
    parser.add_argument('--sd_model_id',       type=str,  default="stabilityai/stable-diffusion-2-inpainting")
    ...
    parser.add_argument('--sd_num_inference_steps', type=int, default=50, help='Stable Diffusion denoising steps')
    ...
    parser.add_argument('--complete_boundary_objects', type=lambda x: str(x).lower() == "true", default=True, help='Whether image-boundary-touching objects should trigger amodal completion')
...
def load_models(gdino_config, gdino_ckpt, sd_model_id, instaorder_ckpt=None, lama_config_path=None, lama_ckpt_path=None, mc_timestep=None, device="cuda"):
    """
    ...
    sd_inpaint_model.safety_checker = None
...
def match_query_class(query_class, classes):
    ...
    for cls in sorted(classes, key=len, reverse=True):
        if cls in query_class or query_class in cls:
            return cls
    return None
...
def check_valid_query(
    ...
    query_pred_score_thresh=0.35, query_mask_size_thresh=0.005, save_interm=False, output_img_dir=None,
):
    """
    ...
    query_class = match_query_class(query_class, classes)
    if query_class is None: return
...
def run_mixed_context_diffusion(
    ...
    # Swap background
    if clean_bkgd_img.shape[:2] != sd_img.shape[:2]:
        clean_bkgd_img = cv2.resize(clean_bkgd_img, (sd_img.shape[1], sd_img.shape[0]))
    ...
    mixed_context_timestep = min(mc_timestep, sd_num_inference_steps - 1)
    mc_output = sd_inpaint_model(
        prompt = sd_prompt,
        image = sd_img_syn,
        mask_image = sd_occ_mask,
        mixed_context = True,
        mixed_context_timestep = mixed_context_timestep,
...
def run_iteration(
    ...
    sides_touched = check_touch_boundary(query_mask)
    query_obj.run_iter = True if (occ_mask.sum() > 0 or (complete_boundary_objects and len(sides_touched) > 0)) else False
...
def run_pipeline(args):
    ...
    gdino_model, sd_inpaint_model, instaorder_model, lama_model = load_models(args.gdino_config, args.gdino_ckpt, args.sd_model_id, args.instaorder_ckpt, args.lama_config_path, args.lama_ckpt_path, args.mc_timestep)
    ...
    for img_filename in tqdm(img_filenames, desc="Iterate images"):
        ...
        subdirs = ["amodal_completions", "amodal_segmentations", "final_segmentations"]
        ...
        img = np.array(img_pil)
        modal_masks = load_modal_masks(args.modal_masks_dir, img_basename) if args.modal_masks_dir else None
        if modal_masks is None:
            ...
            classes = sorted(set(classes + class_names))
        ...
        for mask_id, query_mask in enumerate(masks):
            ...
            # Post-processing. Always save one final mask per detected object:
            # amodal when completion succeeds, otherwise the original SAM modal mask.
            query_class = class_names[mask_id]
            final_mask = query_mask.astype(np.uint8)
            ...
            if query_obj.amodal_segmentation is not None and query_obj.iter_id > 0:
                ...
                final_mask = query_obj.amodal_segmentation.astype(np.uint8)
                mask_type = "amodal"
            ...
            final_segmentation_to_save = Image.fromarray(final_mask * 255).convert("RGB")
            final_segmentation_to_save.save(os.path.join(query_obj.output_img_dir, "final_segmentations", f'{query_class}_{mask_id}.png'))
            mask_metadata.append({
                "id": mask_id,
                "class_name": query_class,
                "score": pred_scores[mask_id],
                "mask_type": mask_type,
                "mask": f"final_segmentations/{query_class}_{mask_id}.png",
                "offset": [x_offset, y_offset],
            })

        img_offset_save_path = os.path.join(output_img_dir, "offsets.json")
        with open(img_offset_save_path, 'w') as fp:
            json.dump(img_offsets_dict, fp, sort_keys=True, indent=4)
        mask_metadata_save_path = os.path.join(output_img_dir, "mask_metadata.json")
        with open(mask_metadata_save_path, 'w') as fp:
            json.dump(mask_metadata, fp, sort_keys=True, indent=4)
